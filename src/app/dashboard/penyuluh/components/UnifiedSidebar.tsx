...
export default function UnifiedSidebar({ userType, onCollapseChange }: SidebarProps) {
  ...
  const getNavItems = (): NavItem[] => {
    switch (userType) {
      ...
      case 'penyuluh':
        return [
          { href: '/dashboard/penyuluh', icon: '/group-white.svg', label: 'Dashboard' },
    ...
    <>
      ...
      <nav className="space-y-2 sm:space-y-3 md:space-y-4">
        {navItems.map((item: NavItem, index: number) => {
          ...
          // Ganti icon otomatis: kalau aktif, hapus "-white" supaya jadi icon hitam
          const iconSrc = active
            ? (item.icon as string).replace('-white', '')
            : (item.icon as string);
